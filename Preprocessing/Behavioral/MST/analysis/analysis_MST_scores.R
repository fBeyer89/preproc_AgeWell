library(psych)
path="/data/pt_02030/preprocessed/mst/"
...
res[res$N_misses_lures==max(res$N_misses_lures),"ID"]#33808.e
max(res$N_misses_new)
res[res$N_misses_new==max(res$N_misses_new),"ID"]#33219.9
max(res$N_misses_old)
res[res$N_misses_old==max(res$N_misses_old),"ID"]#33808.e
...
#exclusion based on CRF-> does not apply to agewell.
...
length(ids)#1 individuals 34115.6 with weird behavior. check and exclude for sensitivity analysis.
