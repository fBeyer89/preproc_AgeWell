...
class NiiWrangler(BaseInterface):
    ...
    def _run_interface(self, runtime):
	    ...
	    #select series number that should be used!! Requires manual intervention (series 23 for 34536.9b)
	    for d in dwi:
		if d['series_num']==23:
		   self.dwi_files = [d[nf]]
        ...
        if len(dwi)>1:
			...
			if d['series_num']==23:
                           print d
                           self.ep_dwi_echo_spacings = 1/(d["bw_per_pix_phase_encode"] * d["acq_matrix_n"])
            else:
